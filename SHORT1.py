...
class HantuSyncEngine:
    ...
    def fetch_market_pool(self, token):
        ...
        # 🎯 한투 오피셜 당일 거래대금 상위 순위 스캔 (가장 돈이 많이 도는 순서)
        ...
        params_vol = {
            ...
            "FID_INPUT_ISCD": "0000", "FID_DIV_CLS_CODE": "0", "FID_SORT_CLS_CODE": "2" # 거래대금 정렬 순서 그대로 수신
        }
        try:
            ...
            if r.status_code == 200:
                ...
                # 🎯 오피셜 서버에서 쏴주는 순위 순서(Order)를 100% 그대로 존중하여 루프를 돕니다.
                for item in output:
                    ...
                    # 만 원 이상 우량주 단타 필터만 적용 (인위적인 보정 결합 없음)
        ...
        return pool # 한투 서버에서 잡힌 만큼만 순수한 알짜 리스트 반환
...
if st.button("🔄 시장 실시간 주도주 새로 불러오기", type="primary", use_container_width=True):
    ...
    with st.spinner("한투 오피셜 서버의 실시간 거래대금 순위를 왜곡 없이 정밀 수집 중..."):
        ...
        if not token:
            ...
            # 🎯 억지로 개수를 채우지 않고 오리지널 마켓 데이터 그대로 스냅샷을 뜹니다.
...
if not df_final.empty:
    ...
    st.info("💡 실시간 조회된 만 원 이상 주도주가 없습니다. 버튼을 눌러 스캔을 시작하시거나, 시장 대기 자금을 보존하십시오.")import streamlit as st
import pandas as pd
import requests
import time
from datetime import datetime, timezone, timedelta
