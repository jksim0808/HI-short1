...
if isinstance(st.session_state.last_pool, list) and len(st.session_state.last_pool) > 0:
    for idx, row in enumerate(st.session_state.last_pool):
        if isinstance(row, tuple) and len(row) == 6:
            ...
            # 🛠️ [핵심 개조] 단타 효과가 가장 좋은 우선순위별 마킹 알고리즘
...
if not df_final.empty:
    ...
    if not selected_rows.empty:
        ...
        # 네이버 차트 전송용 문자열 정화 파이프라인 (대괄호 마크 완전 제거)
...
if selected_ticker:
    ...
    st.info("⬆  상단 순위 리스트에서 원하시는 종목의 [선택] 체크박스를 켜시면 하단에 네이버 오리지널 차트가 표출됩니다.")import streamlit as st
import pandas as pd
import requests
import time
import os
...
# 🖥️ 상단 실시간 통신 진단 모니터
# =====================================================================
st.title("🎯 AI 오전 3단계 스캐너 × 네이버 차트 (All-Live 무결점 버전)")
st.warning(f"📡 **실시간 라인 진단 모니터:** {st.session_state.net_log}")
...
cc1, cc2 = st.columns([4, 1])
with cc1:
    btn_fetch = st.button("🔄 실시간 수급 현황 전체 불러오기 (크래시 가드 작동)", type="primary", use_container_width=True)
with cc2:
    btn_clear = st.button("⚠️ 시스템 세션 초기화", type="secondary", use_container_width=True)
...
# =====================================================================
# 📊 [상단 구역] 수급 테이블 광폭 배치 (★ 크래시 에러 철통 디펜스 개조)
# =====================================================================
st.markdown("### 📊 실시간 수급 종합 순위표 (원하는 종목 앞 체크박스를 선택하세요)")

display_list = []

# 🛠️ [해결] 언팩 에러(ValueError) 방지용 엄격한 구조 검증문 추가
if isinstance(st.session_state.last_pool, list) and len(st.session_state.last_pool) > 0:
    for row in st.session_state.last_pool:
        # 데이터가 안전한 6개 구조 형태를 갖췄을 때만 정밀 압축 해제 실행
        if isinstance(row, tuple) and len(row) == 6:
            t, n, amt, price, ctrt, stat = row
