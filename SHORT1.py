...
# --- [🔥 합체] 2단계: 대표님 제안 기술적 지표 및 퀀트 특성 계산 (Feature Engineering) ---
def calculate_indicators(df):
    if len(df) == 0:
        return df
    
    # [VWAP] 거래량 가중 평균 가격 (당일 트레이더들의 공정 가격선 연산)
    ...
    # [RSI] 5분 주기 기준 과매도/과매수 모멘텀 스코어 연산
    delta = df['Close'].diff()
...
if tick_data and tick_data["Close"] > 0:
    ...
    # 5분 분봉 가상 빌드업 생성 (첫 클릭 시 렌더링 무조건 보장)
    ...
    # 갱신 버튼 누르면 실시간 갱신 누적 (최대 30개 스택 제한)
    ...
    # --- [🔥 합체] 3단계: 매매 시그널 결정 로직 (Rule-based Trading Strategy) ---
    cond_vwap_breakout = latest['Close'] > latest['VWAP']         # 조건 1: 주가가 거래량가중평균선(VWAP) 위에 안착했는가?
    cond_resistance_break = latest['Close'] >= prev_local_high    # 조건 2: 직전 3개 봉의 최고 단기 저항선을 돌파했는가?
    cond_volume_pump = latest['Volume'] > (prev_vol_ma * 1.02)     # 조건 3: 수급(거래량)이 이전 평균치보다 유의미하게 솟구쳤는가?
    
    # 매수/매도 시그널 종합 판정 (과매수 상단 필터 결합)
    is_buy_signal = cond_vwap_breakout and cond_resistance_break and cond_volume_pump and (latest['RSI'] < 78)
    is_sell_signal = latest['RSI'] > 82  # RSI 모멘텀 임계치 초과 시 분할 청산 타점 포착
    ...
    if is_buy_signal:
        ...
        st.info(f"🚨 [과매수 청산 신호] {ticker_input} - RSI {int(latest['RSI'])} 돌파. 단기 슈팅에 따른 분할 익절 타점!")
    else:
    ...
    st.error("🚨 글로벌 시세 오픈 API 통신 불가 상태입니다. 종목코드를 정확히 입력하셨는지 확인 후 잠시 후 다시 시도해 주세요.")import streamlit as st
import pandas as pd
import numpy as np
import requests
from datetime import datetime, timedelta
