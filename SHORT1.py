...
with col2:
    ...
    st.markdown(
        ...
        * **⚡ B급 (후발/방망이짧게)**: 등락률 **+3% 이상 ~ +10% 미만** 종목입니다. 수급은 들어왔으나 탄력이 약하므로 아주 짧은 스캘핑 위주로만 접근합니다.
        * **❌ 매매대상 완전 제외**: 등락률 **+3% 미만(보합/하락)**이거나 한투 시스템상 **투자유의/관리종목** 마크가 붙은 쓰레기 종목은 대표님의 원금을 지키기 위해 표에서 자동 소멸시킵니다.
        """
    )
...
if st.session_state.last_pool:
    for t, n in st.session_state.last_pool:
        ...
        # ⚠️ [수정] 최소 진입 제한 등락률 조건을 5.0에서 3.0으로 변경
        if price_val <= 0 or is_restricted or ctrt_val < 3.0:
            continue
...
if not df_final.empty:
    ...
    # ⚠️ [수정] 안내 메시지의 조건 표기도 5%에서 3%로 변경
    st.info("📊 현재 당일 주도주 조건(+3% 이상 상승 우량주)에 부합하는 매매 대상 종목이 없습니다. 무리한 진입을 피하고 대기자금을 보존하십시오.")
