            ...
            if res.status_code == 200:
                ...
                if result:
                    ...
                    if final_close > 0 and final_close < 1000:
                        final_close = final_close * 1350.0  # 원화 디노미네이션 보정
...
# --- 국내 주요 종목 코드 마스터 맵핑 딕셔너리 ---
# API 통신 속도 절약을 위해 시스템 메모리에 실시간 캐싱합니다.
STOCK_NAME_MAP = {
...
def get_stock_name(ticker):
    """ 코드를 주면 매핑 사전을 찾고, 없는 신규 종목이면 '종목[코드]' 형태로 방어 출력 """
...
# --- 웹 대시보드 인터페이스 초기화 ---
st.set_page_config(page_title="초고속 종목명 스캐너", layout="centered")
...
# =================================================================
# 🛠️ [사이드바] 종목 번호 복사·붙여넣기 등록기
# =================================================================
st.sidebar.markdown("### 📋 종목 번호 멀티 입력")
st.sidebar.caption("종목 코드를 공백이나 쉼표로 구분하여 입력하세요.")
...
for ticker in st.session_state.custom_stock_pool:
    ...
    summary_rows.append({
        "종목코드": ticker,
        "종목명": get_stock_name(ticker),  # 🌟 코드로부터 종목명 추출 가공
...
if summary_rows:
    ...
    for index, row in summary_df.iterrows():
        ...
        # 🌟 헤더 출력을 기존 코드에서 '종목명 (코드)' 구조로 뒤집어 시인성 대폭 강화
