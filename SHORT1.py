...
with col2:
    ...
    st.markdown(
        ...
        * **⚡ B급 (후발/방망이짧게)**: 등락률 **+5% 이상 ~ +10% 미만** 종목입니다. 수급은 들어왔으나 탄력이 약하므로 아주 짧은 스캘핑 위주로만 접근합니다.
        * **❌ 매매대상 완전 제외**: 등락률 **+5% 미만(보합/하락)**이거나 한투 시스템상 **투자유의/관리종목** 마크가 붙은 쓰레기 종목은 대표님의 원금을 지키기 위해 표에서 자동 소멸시킵니다.
        """
    )

with col3:
    st.markdown("### 🛠️ 올바른 실전 사용법")
    st.markdown(
        """
        1. 장중 혹은 주도주 탐색이 필요할 때 하단의 대형 **[🔄 시장 실시간 주도주 새로 불러오기]** 버튼을 클릭합니다.
        2. 과거의 유령 캐시 메모리를 완전히 청소한 뒤 현재 마켓 상태가 투명하게 로딩됩니다.
        3. 만약 장세가 극도로 안 좋거나 살 종목이 없을 때는 억지로 순위를 채우지 않고 **표를 과감히 비운 채 대기 안내**를 출력하여 대기 자금을 안전하게 보존시킵니다.
        4. 표가 활성화되면 **시장투자순위 상위권에 있으면서 [🔥 A급] 마크가 뜬 종목**의 분봉 차트를 열어 타점을 조준하십시오.
        """
    )
...
if st.session_state.last_pool:
    for t, n in st.session_state.last_pool:
        ...
        # 단타 기준에 미달하는 종목(+5% 미만 상승, 보합, 하락주 등)은 리스트에서 완전 즉시 배제
