...
class HantuSyncEngine:
    ...
    def get_token(self):
        if not APP_KEY or not APP_SECRET:
            st.session_state.debug_msg = "❌ 에러: Streamlit Secrets에 한투 APP_KEY 또는 APP_SECRET이 비어있습니다."
            return None
...
if btn_clear:
    ...
    st.session_state.debug_msg = "♻️ 토큰 캐시가 강제 삭제되었습니다."
    st.rerun()
...
if btn_fetch:
    ...
    with st.spinner("마켓 수급 분석 중..."):
        engine = HantuSyncEngine()
        token = engine.get_token()
        
        if not token:
            st.session_state["token_error"] = "토큰 발급 실패"
        else:
            dynamic_pool = engine.fetch_market_pool(token)
            st.session_state.last_pool = dynamic_pool
            
            if dynamic_pool:
                st.session_state.debug_msg = f"🟢 수집 완료: 총 {len(dynamic_pool)}개의 기초 종목 풀을 화면에 직송 출력합니다."
...
if st.session_state.last_pool:
    for t, n in st.session_state.last_pool:
        ...
        # 🛠️ [필터 무조건 해제] 가격, 관리종목, 등락률 필터를 통째로 삭제하여 무조건 수집함
        if ctrt_val >= 25.0:
            rank_grade = "⚡ B급 (과열/추격금지)"
        elif ctrt_val >= 10.0 and ctrt_val < 25.0:
            rank_grade = "🔥 A급 (최우선 대장주)"
        elif ctrt_val >= 1.0 and ctrt_val < 10.0:
            rank_grade = "⚡ B급 (후발/방망이짧게)"
        else:
            rank_grade = "⚪ 보합 이하 / 하락 종목"
...
if not df_final.empty:
    ...
    st.info("📊 데이터 분석은 정상 작동 중이나 연산 처리 데이터가 비어있습니다. 새로고침을 다시 눌러주세요.")
