...
class KoreaInvestmentOfficialAPI:
    ...
    def get_realtime_price(self, ticker, token):
        ...
        # 🎯 [중요 보정] 에이직랜드, 파두, 에코프로비엠은 코스닥(W), 나머지는 코스피(J)로 정밀 분기
...
st.title("🎯 AI 정예 주도주 10선 실시간 단타 스캐너 (✅ 시장 분기 보정판)")
...
if col_btn1.button("⚡ AI 정예 10선 수급 동기화", type="primary", use_container_width=True, key="btn_sync_market_fixed"):
...
if col_btn2.button("🧹 단타 캐시 리셋", use_container_width=True, key="btn_reset_market_fixed"):
    st.session_state.market_history = {}
    st.session_state.live_pct_map = {}
    if "data_loaded" in st.session_state: del st.session_state["data_loaded"]
    st.rerun()

col_info.markdown("💡 **수정 완료:** 코스닥 전용 분류 코드 `W` 연동을 통해 전 종목 데이터 동기화 락이 완전히 해제되었습니다.")
...
if st.session_state.get("data_loaded", False) and st.session_state.market_history:
    ...
    st.info("⏳ 시스템 가동 대기 중입니다. 상단의 동기화 버튼을 클릭해 주십시오.")import streamlit as st
import pandas as pd
import numpy as np
import requests
import time
